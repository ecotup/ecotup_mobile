...
{
    suspend fun getTransaksiByIdTransaksi(idTransaksi : String) = repository.getTransaksiByIdTransaksi(idTransaksi)
    suspend fun updatePointDriver(id : String, point : Int) = repository.updatePointDriver(id, point)

    suspend fun updateStatusTransaksi(idTransaction : String, status : String) = repository.updateStatusTransaksi(idTransaction, status)

    suspend fun updateLatLongDriver(idTransaction : String, latitude : Double, longitude : Double) = repository.updateLatLongDriver(idTransaction, latitude, longitude)
